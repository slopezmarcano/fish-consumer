...
fish_data <- read_csv('data/fish_data.csv') %>%
    select(flavour_no_seasoning, flavour_with_seasoning, texture, spines, next_bite, score)
...
ggqqplot(fish_data_sca$flavour_with_seasoning)
...
#--LINEAR MODEl--#
model <- lm(score ~ flavour_no_seasoning + flavour_with_seasoning + texture + spines + next_bite, data=fish_data_sca)
summary(model)
